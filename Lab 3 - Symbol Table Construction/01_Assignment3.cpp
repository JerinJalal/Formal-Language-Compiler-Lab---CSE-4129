...
int main()
{
    ...
    for (auto &l : lines)
    {
        stringstream ss(l);
        string tok;
        while (ss >> tok) updated_token+=update_token(tok);
    }
    cout << "Step 1: Updated Token :" << endl;
    cout<<updated_token<<endl;

    vector<string> tokens = tokenize(updated_token);
    prep(tokens);

    display();

    generate_modified_stream(tokens);
